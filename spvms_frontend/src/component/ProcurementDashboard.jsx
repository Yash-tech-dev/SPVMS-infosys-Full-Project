...
//  * Working: 
//  * 1. Current user ka role fetch karta hai.
//  * 2. Dono possibilities (with or without ROLE_ prefix) check karta hai taaki access denied na aaye. [cite: 2026-01-06]
//  */
// const user = AuthService.getCurrentUser();
// const role = user?.role;

// if (!user || (role !== "ROLE_PROCUREMENT" && role !== "PROCUREMENT")) {
//     return (
...
import jsPDF from 'jspdf';
import autoTable from 'jspdf-autotable';
// FIX 1: AuthService ko import karna zaroori hai
...
export default function ProcurementDashboard() {
  ...
  // FIX 2: Permission check ko logic ke andar rakhein taaki component crash na ho
  const user = AuthService.getCurrentUser();
  const role = user?.role;

  // Cleanup check: Agar user login nahi hai ya role mismatch hai
  if (!user || (role !== "ROLE_PROCUREMENT" && role !== "PROCUREMENT")) {
    return (
        <div style={{ textAlign: 'center', marginTop: '100px', fontFamily: 'sans-serif' }}>
            <h1 style={{ color: '#e11d48' }}>Access Denied</h1>
            <p>You do not have permission to view the Procurement Dashboard.</p>
            <p style={{ color: '#64748b' }}>Current Role: {role || "No Role Found"}</p>
            <button 
                onClick={() => window.location.href = "/login"}
                style={{ padding: '10px 20px', background: '#1e293b', color: 'white', border: 'none', borderRadius: '5px', cursor: 'pointer' }}
            >
                Return to Login
            </button>
        </div>
  ...
  // ... (Baaki saara state aur functions: stats, prs, fetchDashboardData, handleCreatePR etc. SAME REHNE DEIN) ...
  ...
  const fetchDashboardData = async () => {
    ...
    try {
      const [prRes, poRes, vendorRes] = await Promise.all([
        axios.get("http://localhost:8081/api/pr/my", config),
        axios.get("http://localhost:8081/api/po/my", config),
...
const generatePDF = (po) => {
  ...
  const renderPOs = () => (
    <div style={styles.tableContainer}>
      <h2 style={{marginBottom: '20px'}}>Purchase Orders (PO)</h2>
      {pos.length === 0 ? <p>No Purchase Orders generated yet.</p> : (
      <table style={styles.table}>
        <thead>
          <tr style={styles.trHead}>
